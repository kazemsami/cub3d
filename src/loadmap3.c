...
/*   Updated: 2022/05/19 10:39:14 by ahhammou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_south(t_mlx *mlx, char *str)
{
	if (!mlx->map.stexture)
	{
		...
		mlx->img[SO].img_ptr = mlx_xpm_file_to_image(mlx,
				mlx->map.s_texture, &mlx->img[SO].width, &mlx->img[SO].hieght);
...
int	check_north(t_mlx *mlx, char *str)
{
	if (!mlx->map.ntexture)
	{
		...
		mlx->img[NO].img_ptr = mlx_xpm_file_to_image(mlx,
				mlx->map.n_texture, &mlx->img[NO].width, &mlx->img[NO].hieght);
...
int	check_west(t_mlx *mlx, char *str)
{
	if (!mlx->map.wtexture)
	{
		...
		mlx->img[WE].img_ptr = mlx_xpm_file_to_image(mlx,
				mlx->map.w_texture, &mlx->img[WE].width, &mlx->img[WE].hieght);
...
int	check_east(t_mlx *mlx, char *str)
{
	if (!mlx->map.etexture)
	{
		...
		mlx->img[EA].img_ptr = mlx_xpm_file_to_image(mlx,
				mlx->map.e_texture, &mlx->img[EA].width, &mlx->img[EA].hieght);
